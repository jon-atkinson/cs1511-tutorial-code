...
// on 27/06/24
//
// This program is used to teach working on strings with functions
// it contains char operation functions composed into string op functions
...
int count_lowercase(char string[MAX_CHARS]) {
    ...
    // this works for an empty string too
    // at this point we should have checked each char in string
    return count;
}
...
void delete_following_words(char string[MAX_CHARS]) {
    int i = 0;
    // all this loop does is find the index that we should put the '\0' at
    while (string[i] != '\0' && string[i] != ' ' && i < MAX_CHARS - 1) {
        i++;
    }
